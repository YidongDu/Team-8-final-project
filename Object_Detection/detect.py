...
for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
    ...
    with open('output/%s.csv' % (path[-14:-4]), mode='w') as img_file:
    # Draw bounding boxes and labels of detections
        if detections is not None:
            #print(type(detections))
            #print(detections.shape)
            unique_labels = detections[:, -1].cpu().unique()
#             if unique_labels.shape[0]>=4:
...
#                 print(int(path[-14:-4]))
            ...
            for i, detection in enumerate(detections):
                ...
                y1 = int(((detection[1] - pad_y // 2) / unpad_h) * (img.shape[0]))
                x1 = int(((detection[0] - pad_x // 2) / unpad_w) * (img.shape[1]))
                img_writer.writerow([x1+box_w/2, y1+box_h/2, box_w, box_h, detection[4].item(), detection[-1].item(), detection[5].item(), detection[6].item(), detection[7].item(), detection[8].item(), detection[9].item(), detection[10].item(), detection[11].item(), detection[12].item(), detection[13].item()])

                color = bbox_colors[int(np.where(unique_labels == int(detection[-1]))[0])]
...
#                 Add the bbox to the plot
                ax.add_patch(bbox)
#                 Add label
                plt.text(x1, y1-30, s=classes[int(detection[-1])]+' '+ str('%.4f'%detection[4].item()), color='white', verticalalignment='top',
